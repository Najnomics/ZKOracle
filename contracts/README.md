...
## Project-Specific Notes (ZKOracle)

- Core contract lives in [`src/ZKOracle.sol`](./src/ZKOracle.sol). It mirrors the architecture in the repo root `README.md`
  by maintaining encrypted sum/count, enforcing the indexer/admin permission model, and only decrypting aggregate results.
- Tests are in [`test/ZKOracle.t.sol`](./test/ZKOracle.t.sol) and rely on `util/FheHelper.sol` to simulate CoFHE behavior as
  described in `CONTEXT/fhenix-docs` and `CONTEXT/cofhe-docs`.
- Scripts: [`script/Deploy.s.sol`](./script/Deploy.s.sol) deploys to Fhenix Nitrogen (`chainId 8008148`). Provide
  `ORACLE_INDEXER`, `ORACLE_ADMIN`, `ORACLE_PERIOD`, `ORACLE_MAX_SAMPLES`, and `ORACLE_SUBMISSION_SCALE` env vars before
  running `forge script ... --rpc-url fhenix`.
- Install dependencies with `pnpm install`, then run `forge fmt && forge test` to lint/verify the smart contracts.
- Keep the `CONTEXT/` docs handy (Zcash RPC specs, lightwalletd guides, Fhenix dev docs) when modifying the indexer or
  encrypted workflow so the on-chain logic stays aligned with those references.

### Constructor Parameters

| Parameter              | Purpose                                                                 | Example            |
|------------------------|-------------------------------------------------------------------------|--------------------|
| `_indexer`             | Authorized submitter (off-chain indexer signer)                         | Indexer EOA        |
| `_periodDuration`      | Aggregation window in seconds                                           | `3600`             |
| `_maxSamplesPerPeriod` | Submission cap per window (prevents spam)                               | `256`              |
| `_submissionScale`     | Scaling factor applied before encryption (e.g., 1e4 = 4 decimal places) | `10000`            |
| `admin`                | Owner/admin who can pause, rotate indexer, etc.                         | Deployer EOA       |

Use `setSubmissionsPaused`, `updateMaxSamples`, and `updatePeriodDuration` to fine-tune behaviour without redeploying.
